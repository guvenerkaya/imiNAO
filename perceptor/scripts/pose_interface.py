...
class image_converter:
  ...
  def __init__(self, camera):
    self.img = None
    self.camera = camera
    self.min_part_conf = 0.2

    self.bridge = CvBridge()
    self.image_sub = rospy.Subscriber("/nao_robot/camera/"+self.camera+"/camera/image_raw", Image, self.callback_img)
    self.keypoint_sub = rospy.Subscriber("perceptor/poses", Poses, self.callback_poses)
